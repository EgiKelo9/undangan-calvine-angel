import { WeddingData } from "@/types";

export const weddingData: WeddingData = {
  // ─── Cover Page ────────────────────────────────────────────────────────────
  cover: {
    bgImage: "/images/edit/019.jpg",
    subtitle: "Undangan Pernikahan",
    title: "Calvine & Angel",
    cardLabel: "Kepada Bapak/Ibu/Saudara/i",
    defaultGuestName: "Tamu Undangan",
    invitationText: "Anda telah diundang ke hari bahagia kami",
    buttonText: "Buka Undangan",
  },

  // ─── Hero Section ──────────────────────────────────────────────────────────
  hero: {
    bgImages: [
      "/images/edit/012.jpg",
      "/images/edit/002.jpg",
      "/images/edit/019.jpg",
    ],
    subtitle: "Undangan Pernikahan",
    title: "Calvine & Angel",
    date: "09 Oktober 2026",
    ctaButtonText: "Lihat Undangan",
  },

  countdown: {
    isoDate: "2026-10-09T08:00:00+08:00",
  },

  // ─── Person Section ────────────────────────────────────────────────────────
  greeting: {
    title: "Om Swastyastu",
    text: "Atas Asung Kerta Wara Nugraha Ida Sang Hyang Widhi Wasa/Tuhan yang Maha Esa, kami bermaksud mengundang Bapak/Ibu/Saudara/i, pada Acara Manusa Yadnya Pawiwahan lan Mepandes putra-putri kami sebagai berikut:",
  },

  couple: {
    displayName: "Calvine & Angel",
    weddingDate: "09 Oktober 2026",
    groom: {
      label: "Mempelai Pria",
      fullName: "I Wayan Calvine Radhitya Adnyana, S.M.",
      nickName: "Calvine",
      childRelation: "Putra pertama dari pasangan:",
      fatherName: "I Wayan Adnyana, S.T.",
      motherName: "Ni Kadek Santi Asih (alm)",
      address: "Br. Danginpeken, Sanur, Denpasar Selatan, Denpasar",
      photo: "/images/edit/016.jpg",
    },
    bride: {
      label: "Mempelai Wanita",
      fullName: "Ni Luh Putu Yudhani Angelina Putri, S.Tr.Gz.",
      nickName: "Angel",
      childRelation: "Putri pertama dari pasangan:",
      fatherName: "I Made Gede Sumantra, S.S.",
      motherName: "Luh Arsining",
      address: "Br. Gulingan, Sanur, Denpasar Selatan, Denpasar",
      photo: "/images/edit/017.jpg",
    },
  },

  mepandes: {
    title: "Mepandes",
    participants: [
      "I Wayan Calvine Radhitya Adnyana, S.M.",
      "I Kadek Andrea Surya Pramesta Adnyana, S.Kom.",
      "I Made Adi Arthanaya",
      "Gangga Dewidhasi Aryawan",
    ],
    // relationLabel: "Putra-putri dari pasangan:",
    // parents: {
    //   father: "I Made Gede Sumantra, S.S.",
    //   mother: "Luh Arsining",
    // },
  },

  // ─── Quote Section ─────────────────────────────────────────────────────────
  quote: {
    verse:
      "Grbhnāmi te saubhagatvāya hastam, Mayā patyā jaradastir yathāsah, Bhago aryamā savitā puramdhir, Mahyam tvādurgārhapatyāya devāh.",
    meaning:
      "Dalam sebuah pernikahan kalian disatukan demi sebuah kebahagiaan dengan janji hati untuk saling membahagiakan. Bersamaku engkau akan hidup selamanya karena Tuhan pasti akan memberikan karunia sebagai pelindung dan saksi dalam pernikahan ini. Untuk itulah kalian dipersatukan dalam satu keluarga.",
    source: "(Rgveda : X.85.36)",
  },

  // ─── Event Section ─────────────────────────────────────────────────────────
  eventSection: {
    title: "Lokasi & Waktu",
    description:
      "Dengan penuh kebahagiaan, kami mengundang Bapak/Ibu/Saudara/i untuk hadir menyaksikan dan memberikan doa restu pada rangkaian acara kami.",
    venue: {
      address: 
        "Jl. Batur Sari Gg. II No. 2, Sanur Kauh, Denpasar Selatan, Kota Denpasar, Bali 80227",
      mapsEmbedUrl: 
        "https://www.google.com/maps/embed?pb=!3m2!1sid!2sid!4v1787234666453!5m2!1sid!2sid!6m8!1m7!1sVDotsi2w3jxF0xVVTXDDhQ!2m2!1d-8.684730573793496!2d115.2525975509153!3f2.658887865299448!4f-12.720170131248608!5f0.7820865974627469",
      mapsLink: "https://maps.app.goo.gl/3QKzEUpWUtMW4DYc9",
    },
    leftSchedule: {
      title: "Prosesi",
      date: "Jumat, 09 Oktober 2026",
      timeStart: "10.00",
      timeEnd: "15.00",
      // location: "Nama Venue Puncak Acara",
    },
    rightSchedule: {
      title: "Resepsi",
      date: "Jumat, 09 Oktober 2026",
      timeStart: "18.00",
      timeEnd: "21.00",
      // location: "Nama Venue Resepsi",
    },
  },

  // ─── Gallery Section ───────────────────────────────────────────────────────
  gallery: {
    title: "Momen Bahagia",
    items: [
      { src: "/images/edit/001.jpg", alt: "Momen Bahagia 1" },
      { src: "/images/edit/002.jpg", alt: "Momen Bahagia 2" },
      { src: "/images/edit/003.jpg", alt: "Momen Bahagia 3" },
      { src: "/images/edit/004.jpg", alt: "Momen Bahagia 4" },
      { src: "/images/edit/005.jpg", alt: "Momen Bahagia 5" },
      { src: "/images/edit/006.jpg", alt: "Momen Bahagia 6" },
      { src: "/images/edit/007.jpg", alt: "Momen Bahagia 7" },
      { src: "/images/edit/008.jpg", alt: "Momen Bahagia 8" },
      { src: "/images/edit/009.jpg", alt: "Momen Bahagia 9" },
      { src: "/images/edit/010.jpg", alt: "Momen Bahagia 10" },
      { src: "/images/edit/011.jpg", alt: "Momen Bahagia 11" },
      { src: "/images/edit/012.jpg", alt: "Momen Bahagia 12" },
      { src: "/images/edit/013.jpg", alt: "Momen Bahagia 13" },
      { src: "/images/edit/014.jpg", alt: "Momen Bahagia 14" },
      { src: "/images/edit/015.jpg", alt: "Momen Bahagia 15" },
      { src: "/images/edit/018.jpg", alt: "Momen Bahagia 18" },
      { src: "/images/edit/019.jpg", alt: "Momen Bahagia 19" },
      { src: "/images/edit/020.jpg", alt: "Momen Bahagia 20" },
    ],
  },

  // ─── Gift Section ──────────────────────────────────────────────────────────
  gift: {
    title: "Love Gift",
    description:
      "Doa restu Anda merupakan karunia yang sangat berarti bagi kami. Jika memberi adalah ungkapan tanda terima kasih Anda, kami dengan rendah hati menerima melalui rekening berikut:",
    accounts: [
      {
        bank: "Bank BCA",
        accountNumber: "6700656665",
        accountName: "I Wayan Calvine Radhitya Adnyana",
      },
      {
        bank: "Bank Mandiri",
        accountNumber: "1750003491676",
        accountName: "Ni Luh Putu Yudhani Angelina Putri",
      },
    ],
  },

  // ─── RSVP Section ─────────────────────────────────────────────────────────
  rsvp: {
    title: "Konfirmasi Kehadiran",
    namePlaceholder: "Tulis nama Anda di sini",
    messagePlaceholder: "Tulis ucapan dan doa restu Anda di sini",
    attendanceOptions: ["Hadir", "Tidak Hadir", "Belum Konfirmasi"],
  },

  // ─── Footer Section ────────────────────────────────────────────────────────
  footer: {
    brandName: "Pawiwahan",
    brandTagline: "Calvine & Angel",
    designer: "Aryadana Baraja Putra",
    year: new Date().getFullYear(),
    // instagram: "@karsaa.id",
    // tiktok: "@karsaa.id",
    // whatsapp: "+62 897-7289-923",
  },

  // ─── Ornaments ─────────────────────────────────────────────────────────────
  ornaments: {
    balineseHeader1: "/assets/balinese-header-1.svg",
    balineseHeader2: "/assets/balinese-header-2.svg",
    balineseHeader3: "/assets/balinese-header-3.svg",
    balineseHeader4: "/assets/balinese-header-4.svg",
    floralBackground: "/assets/floral-background.svg",
  },

  // ─── Audio ─────────────────────────────────────────────────────────────────
  audio: {
    src: "/audio/asmalibrasi.mp3",
    title: "Asmalibrasi",
    artist: "Soegi Bornean",
  },
};
